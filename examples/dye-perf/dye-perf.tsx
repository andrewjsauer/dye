...
import {
	render, Box, Text, type FrameEvent,
} from '../../src/index.js';
...
const stats = {
	count: 0, total: 0, max: 0, lastMs: 0,
};
	...
	return (
		<Box flexDirection='column' padding={1} gap={1} borderStyle='round'>
			<Text bold color='cyan'>
				Dye Performance Demo
			</Text>
			<Box flexDirection='column'>
				<Text>
					Frames rendered: <Text color='yellow'>{stats.count}</Text>
				</Text>
				<Text>
					Last frame: <Text color='green'>{stats.lastMs.toFixed(2)}ms</Text>
				</Text>
				<Text>
					Average: <Text color='green'>{avgMs.toFixed(2)}ms</Text>
				</Text>
				<Text>
					Max: <Text color='red'>{stats.max.toFixed(2)}ms</Text>
				</Text>
			</Box>
...
render(<App />, {
	onFrame(event: FrameEvent) {
		...
		if (event.durationMs > stats.max) {
			stats.max = event.durationMs;
		}
	},
});
