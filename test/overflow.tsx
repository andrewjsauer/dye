...
const box = (text: string, options?: Options): string => boxen(text, {
	...options,
	borderStyle: 'round',
});

const clipX = (text: string, columns: number): string => text
	.split('\n')
	.map(line => sliceAnsi(line, 0, columns).trim())
	.join('\n');

test('overflowX - single text node in a box inside overflow container', t => {
	const output = renderToString(<Box width={6} overflowX='hidden'>
...
test('overflowX - single text node inside overflow container with border', t => {
	const output = renderToString(<Box width={6} overflowX='hidden' borderStyle='round'>
		<Box width={16} flexShrink={0}>
			<Text>Hello World</Text>
		</Box>
	</Box>);
...
test('overflowX - multiple text nodes in a box inside overflow container with border', t => {
	const output = renderToString(<Box width={8} overflowX='hidden' borderStyle='round'>
		<Box width={12} flexShrink={0}>
			<Text>Hello </Text>
			<Text>World</Text>
		</Box>
...
test('overflowX - multiple text nodes in a box with border inside overflow container', t => {
	const output = renderToString(<Box width={8} overflowX='hidden'>
...
test('overflowY - single text node inside overflow container', t => {
	const output = renderToString(<Box height={1} overflowY='hidden'>
		<Text>Hello{'\n'}World</Text>
	</Box>);
...
test('overflowY - single text node inside overflow container with border', t => {
	const output = renderToString(<Box width={20} height={3} overflowY='hidden' borderStyle='round'>
		<Text>Hello{'\n'}World</Text>
	</Box>);
...
test('overflowY - multiple boxes inside overflow container', t => {
	const output = renderToString(<Box height={2} overflowY='hidden' flexDirection='column'>
		<Box flexShrink={0}>
			<Text>Line #1</Text>
		</Box>
		<Box flexShrink={0}>
...
test('overflowY - box above top edge of overflow container with border', t => {
	const output = renderToString(<Box width={7} height={3} overflowY='hidden' borderStyle='round'>
...
test('overflow - single text node inside overflow container', t => {
	const output = renderToString(<Box paddingBottom={1}>
...
test('overflow - box intersecting with top left edge of overflow container', t => {
	const output = renderToString(<Box width={4} height={4} overflow='hidden'>
...
test('out of bounds writes do not crash', t => {
	...
	const expected = boxen('', {
		...
		.split('\n')
		.map((line, index) => index === 0 || index === 9
			? line
...
test('overflowX - single text node in a box inside overflow container - concurrent', async t => {
	const output = await renderToStringAsync(<Box width={6} overflowX='hidden'>
		<Box width={16} flexShrink={0}>
			<Text>Hello World</Text>
		</Box>
	</Box>);
...
test('overflowY - single text node inside overflow container - concurrent', async t => {
	const output = await renderToStringAsync(<Box height={1} overflowY='hidden'>
		<Text>Hello{'\n'}World</Text>
	</Box>);
	t.is(output, 'Hello');
});
...
test('overflow - single text node inside overflow container - concurrent', async t => {
	const output = await renderToStringAsync(<Box paddingBottom={1}>
		<Box width={6} height={1} overflow='hidden'>
			<Box width={12} height={2} flexShrink={0}>
				<Text>Hello{'\n'}World</Text>
			</Box>
