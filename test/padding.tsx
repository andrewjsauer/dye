...
test('padding', t => {
	const output = renderToString(<Box padding={2}>
		<Text>X</Text>
	</Box>);
...
test('padding Y', t => {
	const output = renderToString(<Box paddingY={2}>
		<Text>X</Text>
	</Box>);
...
test('padding top', t => {
	const output = renderToString(<Box paddingTop={2}>
		<Text>X</Text>
	</Box>);
...
test('padding bottom', t => {
	const output = renderToString(<Box paddingBottom={2}>
		<Text>X</Text>
	</Box>);
...
test('apply padding to text with newlines', t => {
	const output = renderToString(<Box padding={1}>
		<Text>Hello{'\n'}World</Text>
	</Box>);
	t.is(output, '\n Hello\n World\n');
});
...
test('apply padding to wrapped text', t => {
	const output = renderToString(<Box padding={1} width={5}>
...
test('text wrapping respects paddingX with flexGrow', t => {
	...
	const output = renderToString(<Box width={40} borderStyle='round'>
		<Box paddingX={2}>
			<Box marginLeft={2}>
				<Text>•</Text>
...
test('padding - concurrent', async t => {
	const output = await renderToStringAsync(<Box padding={2}>
		<Text>X</Text>
	</Box>);
